...
router.post(modelIName, async (req, res) => {
  ...
  router.delete(modelIName, async (req, res) => {
    ...
    let reqParams = req.query ?? (req.params ?? {})
    console.log('client Data======2',reqParams,reqData,req.query ?? req.params)

    if ((Object.keys(reqParams).length == 0)) {
      return res.status(501).send("Search Query is null ?");
    }

    try {
      return await updateSaleit(reqParams,operation,deleteOperation).then((modelData)=>{
        res.set(_setResponseHeader)
        return res.status(modelData.status).send(modelData.data);                 
      })
