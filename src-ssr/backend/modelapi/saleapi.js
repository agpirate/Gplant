...
import { createcust,updatecust } from "app/src-ssr/services/modalServices/custServices";
const modelI = custModel
const modelIName = "/cust"

import { creategood,updategood } from "app/src-ssr/services/modalServices/goodServices";
const modelII = goodModel
const modelIIName = "/goods"

import { createproduct,updateproduct } from "app/src-ssr/services/modalServices/productServices";
const modelIII = productModel
const modelIIIName = "/product"
...
router.post(modelIName, async (req, res) => {
  ...
  //----------------
  return await createcust(reqData,reqData['onplayOps'] ?? 'CreateRowItem').then((modelData)=>{
...
router.put(modelIName, async (req, res) => {
      ...
      //------------
      reqData['userId']=userId
      return await updatecust(reqData,reqData['onplayOps'] ?? 'UpdateRowItem').then((modelData)=>{
        res.set(_setResponseHeader)
        return res.status(modelData.status).send(modelData.data);                 
      })
});
...
router.post(modelIIName, async (req, res) => {
  ...
  reqData['userId']=userId
  //----------------
  return await creategood(reqData,reqData['onplayOps'] ?? 'CreateRowItem').then((modelData)=>{
    res.set(_setResponseHeader)
    return res.status(modelData.status).send(modelData.data);                 
  })
});
...
router.put(modelIIName, async (req, res) => {
      ...
      //------------
      reqData['userId']=userId
      return await updategood(reqData,reqData['onplayOps'] ?? 'UpdateRowItem').then((modelData)=>{
        res.set(_setResponseHeader)
        return res.status(modelData.status).send(modelData.data);                 
      })
});
...
router.post(modelIIIName, async (req, res) => {
  ...
  reqData['userId']=userId
  //----------------
  return await createproduct(reqData,reqData['onplayOps'] ?? 'CreateRowItem').then((modelData)=>{
...
router.put(modelIIIName, async (req, res) => {
      ...
      //------------
      reqData['userId']=userId
      return await updateproduct(reqData,reqData['onplayOps'] ?? 'UpdateRowItem').then((modelData)=>{
        res.set(_setResponseHeader)
        return res.status(modelData.status).send(modelData.data);                 
      })
});
