...
router.delete("/leavereq", async (req, res) => {
  ...
  //else{}
  try {
    let response = await dOps(leavereqModel, reqParams["id"]); //send Id_value only
    if (response.status == 200) {
      res.set(resHeader);
      return res.send(response["data"]);
    } else {
...
router.delete("/report", async (req, res) => {
  ...
  //else{}
  try {
    let response = await dOps(reportModel, reqParams["id"]); //send Id_value only
    if (response.status == 200) {
      res.set(resHeader);
      return res.send(response["data"]);
    } else {
...
router.delete("/plan", async (req, res) => {
  ...
  //else{}
  try {
    let response = await dOps(planModel, reqParams["id"]); //send Id_value only
    if (response.status == 200) {
      res.set(resHeader);
      return res.send(response["data"]);
    } else {
...
async function update_employee(reqParams,reqData){
  ...
  console.log( "updating enter Updated-0",reqData,findBy);
  ...
  try {
    ...
    return await empyModel
      ...
      .limit(limits)
      .then((modelQA) => {
        console.log( "updatingf",modelQA,reqData);
...
router.put("/empy", async (req, res) => {
  ...
  multer(getFileOptions()).single("profile")  (req, res, async (err) => {
    ...
    if(!nul.includes(filesMeta)){ 
      reqData=Object.assign({},filesMeta)
    }
//else{reqData = nul.includes(reqData) ? {} :reqData}
...
router.delete("/empy", async (req, res) => {
  ...
  try {
    let response = await dOps(empyModel, reqParams["id"]); //send Id_value only
    if (response.status == 200) {
      res.set(resHeader);
...
//------------------------------------------attend
async function compAttend(reqData){
...
router.post("/attend", async (req, res) => {
  ...
  //-----------
  //get the month ( )
let reqDataa = await compAttend(reqData)
  ...
  try {
    ...
    // Save the new customer to the database
    let Doc = new attendModel(reqDataa);
    //////console.log(Doc, reqData, findBy, "PPPPPPPPPP");
    await Doc.save()
      .then((response) => {
        if (Object.keys(response).length) {
...
router.delete("/attend", async (req, res) => {
  ...
  try {
    let response = await dOps(attendModel, reqParams["id"]); //send Id_value only
    if (response.status == 200) {
      res.set(resHeader);
      return res.json(response["data"]); //res.json({ data: response['data'], status: "success" });
    } else {
...
router.delete("/comt", async (req, res) => {
  ...
  try {
    let response = await dOps(comtModel, reqParams["id"]); //send Id_value only
    if (response.status == 200) {
      res.set(resHeader);
      return res.json(response["data"]); //res.json({ data: response['data'], status: "success" });
    } else {
...
router.delete("/post", async (req, res) => {
  ...
  try {
    let response = await dOps(postModel, reqParams["id"]); //send Id_value only
    if (response.status == 200) {
      res.set(resHeader);
      return res.json(response["data"]); //res.json({ data: response['data'], status: "success" });
    } else {
