...
import {productModel} from  "app/src-ssr/backendCore/models/saleModels"
//---------
var thisModel = productModel
...
async function createproduct(reqData,columnOperation =null,columnsubOperation =null){
    try {
...
async function updateproduct (reqData,columnOperation =null,columnsubOperation =null){
    try {
      //------------Preparing finding Query
      let findBy={} 
...
//ItemQ,quantity(calculated(cylAmount && M3) or 1),unitCost or cost(unitCost,ImportCost++),financeCost(totalCost = ItemQs x unitCost)
//Onstore = ItemQs - takenBy(Q)
//saling :_ ItemQs(s), unitPrice(s).... itemPrice = (ItemQs(s) * quantity) * unitPrice(s) && sale Only from takenBy Amount
async function _primaryOperations(modelQA,reqData) {
  ...
  try{
    ...
    if (Number(_checkObj("cylAmount")[2]) && _checkObj("UoM")[2]) { //For Product Pricing using Litter or Kg, by UnitPrice
    ...
    console.log('\n Resource Quantity is ==== Interm OF quantity\n')
     quantityMeasured = Number(_checkObj('quantity')[2]) == 0 ? 1 :Number(_checkObj('quantity')[2])// nul.includes(reqData["quantity"]) ? 1 : Number(reqData["quantity"]); //unit price give as number or set_of_object like below
    }
