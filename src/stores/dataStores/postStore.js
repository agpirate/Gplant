...
const API_URL = process.env.API_IP_PORT+"/chat"; //const API_URL = `${import.meta.env.API_URL}/users`;
...
const _model_mainURL="/post"
const STORE_NAME ="postStore";
...
async function responseHandler(response) { ////((((((((RESPONSE HANDLER  [.data / .statusText]))))))))
  let errorMessage='New Response Received.'
  // console.log(response,"Response Data")
  try {
    errorMessage='Checking response status(200) and data' 
    let data = response.data ?? false;
    if (_okStatus.includes(response.status ?? false) && data) { 
      errorMessage='Checking Json Header' 
      ...
      errorMessage='Checking response status(!200) and data' 
      if (_serviceStatus.includes(response.status)) {     }
      else if (_serverokStatus.includes(response.status)) {     }
    }
} catch(error) {errorMessage=Object.assign({},error);console.log(error,"Authentication Error ??") }
...
export const postStore = defineStore(STORE_NAME, () => {
  //state
  let Datas = ref([]); //
  let status_Rows = ref('');
