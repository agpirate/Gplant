    ...
    if (body != null) {
      ...
      try {
        ...
        return await responseHandler(response).then((HandledRESP) => { return{status:true,data:HandledRESP} }, // Handling Friendly Errors of the response
                                                (error) => {return {status:true,data:error}}); // the friendly errors
      } catch (error) {
      ...
      try {
        ...
        return await responseHandler(response).then((HandledRESP) => { return{status:true,data:HandledRESP} }, // Handling Friendly Errors of the response
                                                  (error) => {return {status:false,data:error}}); // the friendly errors
      } catch (error) {
        return {status:false,data:error};
      } 
    }
...
async function responseHandler(response) { ////((((((((RESPONSE HANDLER  [.data / .statusText]))))))))
  console.log(response,'Profile Store')
  ...
  try {
    ...
    if(isJson){
      ...
      if (_okStatus.includes(response.status) && data) {  
...
//--------------------------------------------Helpers
async function requestHeader(url='',_model='profile') {
  //Inject Header
