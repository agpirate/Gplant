        ...
        <q-card-section class="q-pt-xs col-auto row q-gutter-sm" >
          ...
          <span class="text-weight-bold  co-auto row justify-end q-my-sm">
              <div class="col"> 
                ...
                <q-separator inset/>
            <div class="col-auto transparent q-pa-xs q-gutter-xs row  items-start" >
              <q-item
...
import { reportStore } from "stores/dataStores/reportStore"; //report Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //report Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { reportSchema } from "src/composables/schemas/serviceSchemas";
...
import {useAssetFilter} from "src/composables/filters/reportfilter"
var {
...
_thisModel.value = "report"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = reportSchema

 const reportService = reportStore();
const _thisService = thisStore();
const _thisapiUrl ='/services/report'
...
return reportService.getstatus_Rows}) 
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(reportService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(reportService.getDatas)
});
...
onMounted(async () => {
  reportService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  reportService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
let search = ref("");
let filterDays = ref({ min: 2, max: 9 })
