<template id="full-page">
  ...
  <div class="col row q-pa-xs q-gutter-xs" style="max-height: 90vh" v-if="_this_acctype ?? false">
    <div  class="col-grow column rounded"
...
import { ref, reactive, computed,onUnmounted, onMounted, onBeforeMount, watch } from "vue";
...
import { leavereqStore } from "stores/dataStores/leavereqStore"; //leavereq Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //leavereq Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { leavereqSchema } from "src/composables/schemas/serviceSchemas";
...
import {useAssetFilter} from "src/composables/filters/leavereqfilter"
var {
...
_thisModel.value = "leavereq"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = leavereqSchema

 const leavereqService = leavereqStore();
const _thisService = thisStore();
const _thisapiUrl ='/services/leavereq'
...
return leavereqService.getstatus_Rows}) 
...
async function Reset_this(Ops,rowIndex){
   ...
   __thisOpsStatus.value=false
    ...
    console.log(_this.value,'Reset', _this_Rows.value)
...
async function _setDefaults(){
    ...
    console.log(_thisDefault.value,'defaulttttttingww',Objprops)
    return true
  }
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(leavereqService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(leavereqService.getDatas)
});
...
onMounted(async () => {
  leavereqService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  leavereqService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
const filter = computed(() => {
  return {
    search: search.value,
    annual: checkBoxValue.group_I.annual, //StoreStatusFilter
    sick: checkBoxValue.group_I.sick,
    demand: checkBoxValue.group_I.demand,
  };
}); //with out return
...
let itemDetailColumns = reactive([
  ["department", "position", "fupdatedAt"],
  [
    "companyID",
    "planID",
    "leaveDate",
