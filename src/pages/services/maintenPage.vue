<template id="full-page">
  ...
  <div class="col row q-pa-xs q-gutter-xs" style="height: 90vh" v-if="_this_acctype ?? false">
    ...
    <!--- Profiling...-->
    <div
      class="col-auto column rounded shadow-2"  style="height: 90vh" v-show="selected[selected.length - 1]"
    >
      <q-card
        ...
        <q-card-section class="q-gutter-col-xs fit q-ma-none q-pa-none">
          ...
          <q-card class="col-auto q-pa-none q-ma-none" style="" flat bordered>
            ...
            <q-card-section class="q-pt-none row q-gutter-xs">
              <q-chip dense class="col-6" icon="event"> </q-chip>
              <q-chip dense icon="bookmark"> </q-chip>
              <q-chip
                dense
                class="col-6"
                icon="alarm"
              ...
              />
              <q-chip dense icon="directions">{{ itemDetail["userID"] }}</q-chip>
            </q-card-section>
                      ...
                      <q-item-section top>
                        ...
                        >
                          <div lines="1" v-for="(it, kk) of ite" :key="it" class="col">
                            ...
                            <q-item-label caption class="q-px-lg">
...
import { maintenStore } from "stores/dataStores/maintenStore"; //mainten Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //mainten Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { maintenSchema } from "src/composables/schemas/serviceSchemas";
...
import {useAssetFilter} from "src/composables/filters/maintenfilter"
var {
...
_thisModel.value = "mainten"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = maintenSchema

 const maintenService = maintenStore();
const _thisService = thisStore();
const _thisapiUrl ='/services/mainten'
...
return maintenService.getstatus_Rows}) 
...
// let visibleColumns = ref([]); //Hold List of In
 invisibleColumns.value = ["maintenanceStatus", "position", "companyID"]; //defualt hidden columns
if ($q.screen.lt.md) {
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(maintenService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(maintenService.getDatas)
});
...
onMounted(async () => {
  maintenService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  maintenService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
let checkBoxLabel = {
  ...
  group_I: ["Maintained", "Nonemaint", "Issued"],
};
//***************** ALTER THOSE ABOVE LABELS(OPTIONAL) &  THE sS1/2/3(must)*****/

let checkBoxValue = reactive({
  group_I: { Maintained: true, Nonemaint: true, Issued: true },
});
...
const filter = computed(() => {
  return {
    search: search.value,
    Maintained: checkBoxValue.group_I.Maintained, //StoreStatusFilter
    Nonemaint: checkBoxValue.group_I.Nonemaint,
...
function filterMethod(rows, terms) {
  ...
  const filteredRows = rows.filter((row, i) => {
    ...
    let sS1 =
      quickFilter.quickOOptions.Maintained && row.requestStatus == "Maintained"; //holding toggle_quicx search
    let sS2 =
      quickFilter.quickOOptions.Nonemaint &&
      row.requestStatus == "None maintained";
    let sS3 = quickFilter.quickOOptions.Issued && row.requestStatus == "issued";
...
let itemDetailColumns = reactive([
  ["createdAt", "requestId", "requestStatus"],
  ["companyID", "department", "position", "requestStatus", "companyID"],
  ["maintenanceStatus"],
]);
