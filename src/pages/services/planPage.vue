...
import { planStore } from "stores/dataStores/planStore"; //plan Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //plan Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { planSchema } from "src/composables/schemas/serviceSchemas";
...
import {useAssetFilter} from "src/composables/filters/planfilter"
var {
...
_thisModel.value = "plan"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = planSchema

 const planService = planStore();
const _thisService = thisStore();
const _thisapiUrl ='/services/plan'
...
return planService.getstatus_Rows}) 
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(planService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(planService.getDatas)
});
...
onMounted(async () => {
  planService.asyncAnualData().then((response) => {
    if (response) {
...
const filter = computed(() => {
  return {
    search: search.value,
    approved: checkBoxValue.group_I.approved, //StoreStatusFilter
...
function filterMethod(rows, terms) {
  ...
  const filteredRows = rows.filter((row, i) => {
    ...
    _group0 = quickFilter.quickOOptions.approved && row.gmStatus == "Vr" ? true : _group0; //holding toggle_quicx search
    _group0 = quickFilter.quickOOptions.issued && row.gmStatus == "" ? true : _group0;
