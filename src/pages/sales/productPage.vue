<template id="full-page">
  ...
  <div class="col row  q-gutter-xs " style="height: 90vh" v-if="_this_acctype ?? false">
...
<!-- {{ _this_Rows}}  -->
        ...
        <q-card-section class="q-pt-xs col-auto row q-gutter-sm" v-if="_pageSettings.showMax">
    ...
    <!----------------------------------VISUALITH-->
    <div
    class="col row rounded"
     v-else >

    <div class="col column" style="background:rgba(0, 68, 255, 0.5);">
     <!--selected-->
     <BChart style="max-height:88vh;"
        class="col" 
        :selected="selected"
        :_thisModel="_thisModel"
        columntoAnalysis="productName"
        analysiserColumn="storeStatus"
        :analysiserItemValues="[
          ['tQs', 'tQCs'],
          ['tQt', 'tQCt'],
        ]"
        :analysiserColumnValues="[
          { label: 'StoreStatus', value: 'storeStatus' },
          { label: 'Quantity', value: 'tQ' },
          { label: 'Cost', value: 'tQC' },
        ]"
        :storeService="productService.getyearDatas"
      />
    </div>
 
              <!--- Profiling...--> 
     <div  class="col-3 column rounded shadow-2 text-left items-right justify-right"  > 
      <q-card
...
<q-page-sticky position="bottom" :offset="[1, 1]">
 
  <q-btn icon="clear" :label="_pageSettings.showMax ? 'Max View' : 'Min View'" @click="_pageSettings.showMax = !_pageSettings.showMax"  />

<q-btn-toggle 
              v-model="_pageSettings.vw_i"
              toggle-color="dark"
              color="green"
              rotate="90"
              :options="[
                {label: 'Item List', value: true},
...
import { productStore } from "stores/dataStores/productStore"; //product Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //product Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { productSchema } from "src/composables/schemas/saleSchemas";
...
import {useAssetFilter} from "src/composables/filters/productfilter"
var {
...
let _pageSettings=ref({'vw_i':true,showForm:false,showMax:false})
...
return productService.getstatus_Rows}) 
...
lockedColumns.value =["userID","companyID","_Stage_"]// ref([ "userID","companyID"]); //are blacklist of columns tobe not shown during the Registerations..form submitting
let _xDisplayDefaulted =ref([
  'userID','companyID','_Stage_','sold_Price','sold_Quantity'
])
...
//  visibleColumns.value = ref([]); //Hold List of In
 invisibleColumns.value = [
  ...
  "productionRate",
  "gmStatus",
  "companyID","subCat","model","ItemQs","qualityStatus"
]; //defualt hidden columns
if ($q.screen.lt.sm) {
  //invisibleColumns.value.push("storeStatus");
  invisibleColumns.value.push("cylAmount");
  invisibleColumns.value.push("UoM");
  invisibleColumns.value.push("cylOwner");
...
if ($q.screen.lt.lg) {
  ...
  invisibleColumns.value.push("cylOwner");
  //invisibleColumns.value.push("storeStatus");
  //invisibleColumns.value.push("productName");
  invisibleColumns.value.push("gmStatus");
  // invisibleColumns.value.push("assetOf");
  //invisibleColumns.value.push("catagories");
  invisibleColumns.value.push("subCat");
  invisibleColumns.value.push("cost");
  //invisibleColumns.value.push("storeStatus");
  invisibleColumns.value.push("productName");
  invisibleColumns.value.push("qualityStatus");
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(productService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(productService.getDatas)
});
...
async function Create_this() {  //UpdateRowItem User DAta with no graphic to update
    ...
    return await Crud_this.createData()
        .then(async (response) => {
          if (response) {
            _this_Rows.value.unshift(response)
...
onMounted(async () => {
  productService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  productService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
let itemDetailColumns = reactive([
  ...
  ["saleStatus", "storeStatus", "QuarterPlan"],
]);
...
//3)--------------Quick_item Filter ( Simple Search/Quick)d
let checkBoxLabel = {
  //items_value to be filtered..
  group_I: ["Oxygen", "Carbon", "Acety","Aceton","N2_l","N2_g"],
  group_II: ["Onstore","Sold"],
  group_III: ["Payed","Credit",'all'],
};
//***************** ALTER THOSE ABOVE LABELS(OPTIONAL) &  THE sS1/2/3(must)*****/

let checkBoxValue = reactive({
  group_I: { Oxygen: true, Carbon: true, Acety: true, Aceton: true, N2_g: true, N2_l: true},
  group_II: { Onstore: true,Sold:false,GasPlus:true },
  group_III: { Payed: true,Credit:true,all:true},
});
...
const filter = computed(() => {
  return {
    search: search.value,
    Oxygen: checkBoxValue.group_I.Oxygen, //StoreStatusFilter
    Carbon: checkBoxValue.group_I.Carbon,
    Acety: checkBoxValue.group_I.Acety,
    Aceton: checkBoxValue.group_I.Aceton,
    N2_l: checkBoxValue.group_I.N2_l,
    N2_g: checkBoxValue.group_I.N2_g,
    //------------------
    Onstore: checkBoxValue.group_II.Onstore,
    Sold: checkBoxValue.group_II.Sold,

    Payed: checkBoxValue.group_III.Payed,
    Credit: checkBoxValue.group_III.Credit,
    all: checkBoxValue.group_III.all,

    GasPlus: checkBoxValue.group_II.GasPlus,
