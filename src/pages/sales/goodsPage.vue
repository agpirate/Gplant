<template id="full-page">
  ...
  <div class="col row  q-gutter-xs " style="height: 90vh" v-if="_this_acctype ?? false">
    ...
    >
        ...
        <q-card-section
          class="q-pa-none col column  wrap scroll"
        >
          <!----------------------------------------- Table View...-->
          <div class="col row ">
            <q-table 
              class="col fontcstyle sticky-table "
              :dense="true"
              :rows="_this_Rows"
              :columns="columns"
...
<!-- {{ _this_Rows}}  -->
    ...
    <div class="col column">
     ...
     <BChart style="max-height:88vh;"
        ...
        :storeService="goodsService.getyearDatas"
      />
    </div>
...
import { goodsStore } from "stores/dataStores/goodsStore"; //goods Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //goods Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { goodsSchema } from "src/composables/schemas/saleSchemas";
...
import {useAssetFilter} from "src/composables/filters/goodsfilter"
var {
...
_thisModel.value = "goods"; // the Vue_Page_DataModel (Listing Collection Name)
// var _thisModelHeader = ref(" Goods Managment"); // the Vue_Page_Data_Headers(Descriptions...)
_this_Schema.value = goodsSchema


const goodsService = goodsStore();
const _thisService = thisStore();
const _thisapiUrl ='/sale/goods'
...
return goodsService.getstatus_Rows}) 
...
if ($q.screen.lt.lg) {
  ...
  invisibleColumns.value.push("plantName");
  invisibleColumns.value.push("gmStatus");
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(goodsService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(goodsService.getDatas)
});
...
onMounted(async () => {
  goodsService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  goodsService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
let checkBoxLabel = {
  ...
  group_I: ["o2", "co2", "c2h2","useable","facility"],
  group_II: ["Issued","Stored","Sold"],
  group_III: ["Payed","Credit",'all'],
};
//***************** ALTER THOSE ABOVE LABELS(OPTIONAL) &  THE sS1/2/3(must)*****/

let checkBoxValue = reactive({
  group_I: { c2h2:true,o2: true, co2: true,useable: false,facility:true },
  group_II: { Issued:true,Stored:true,Sold:true },  
...
const filter = computed(() => {
  return {
    search: search.value,
    c2h2: checkBoxValue.group_I.c2h2, //StoreStatusFilter
    o2: checkBoxValue.group_I.o2,
    co2: checkBoxValue.group_I.co2,
    useable: checkBoxValue.group_I.useable,
    facility: checkBoxValue.group_I.facility,
    //---
    Issued: checkBoxValue.group_II.Issued,
    Stored: checkBoxValue.group_II.Stored,
...
let itemDetailColumns = reactive([
  ...
  ["goodsName", "ItemQs", "UoM", "cost"],
  ["financeStatus", "supplier", "storeStatus", "saleStatus", "financeApStatus"],
]);
