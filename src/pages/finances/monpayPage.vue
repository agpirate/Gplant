<template id="full-page">
  ...
  <div class="col row q-pa-xs q-gutter-xs" style="height: 90vh" v-if="_this_acctype ?? false">
    <div
    class="col-xs-12 col-sm-12 col-md-grow col-lg-auto col-xl-auto column rounded"
    style="height: 90vh"
    >
            ...
            >
              ...
              <template #header-cell="props">
                <q-th
                  ...
                  class="text-bold text-center fontcstyle"
                  :props="props"
  ...
  <div
    ...
    <div
      class="col-xs-12 col-sm-grow col-md-grow col-lg-grow col-xl-grow column"
      style="height: 90vh"
    >
      <!--selected-->
      <BChart
        ...
        :storeService="monpayService.getyearDatas"
      />
...
import { monpayStore } from "stores/dataStores/monpayStore"; //monpay Store ( Main Store)
...
import { monpaySchema } from "src/composables/schemas/financeSchemas";
...
import {useAssetFilter} from "src/composables/filters/monpayfilter"
var {
...
_thisModel.value = "monpay"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = monpaySchema

 const monpayService = monpayStore();
const _thisService = thisStore();
const _thisapiUrl ='/finance/monpay'
...
return monpayService.getstatus_Rows}) 
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(monpayService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(monpayService.getDatas)
});
...
onMounted(async () => {
  monpayService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  monpayService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
