<template id="full-page">
  ...
  <div class="col column q-pa-xs q-gutter-xs" v-if="_this_acctype ?? false">
    <div
      class="col column rounded"
    >
            ...
            >
              ...
              <template #header-cell="props">
                <q-th
                  ...
                  class="text-bold text-center  bg-blue-10 text-white"
                  :props="props"
      ...
      >
        <BChartStatment
          ...
          :analysiserColumnValues="[
            { label: 'Maincat', value: 'storeStatus' },

            { label: 'measurmentOne', value: 'tI' },
            { label: 'measurmentTwo', value: 'tX' },
          ]"
          :storeService="statmentService.getyearDatas"
        />
      </div>
      <div
        class="col-auto shadow-2 rounded-borders text-secondary q-pa-none q-px-md column transparent"
        style="
          background: rgba(192, 192, 207, 0.493);
          border: 0.5px solid rgba(255, 255, 255, 1);
        "
...
import { statmentStore } from "stores/dataStores/statmentStore"; //statment Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //statment Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { statmentSchema } from "src/composables/schemas/financeSchemas";
...
// import {useAssetFilter} from "src/composables/filters/statmentfilter"
// var {
...
_thisModel.value = "statment"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = statmentSchema

 const statmentService = statmentStore();
const _thisService = thisStore();
const _thisapiUrl ='/finance/statment'
...
return statmentService.getstatus_Rows}) 
...
//------column invisiblity on table_
 ...
 invisibleColumns.value = ["companyID","_stage_"]; //defualt hidden columns
if ($q.screen.lt.md) {
  invisibleColumns.values.push("storeStatus");
}
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(statmentService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(statmentService.getDatas)
});
...
onMounted(async () => {
  statmentService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  statmentService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
let quickFilterOptions = {
  ...
  PD_G_Inc: "Paid Goods Income",
  PD_P_Inc: "Paid Product Income",
  RL_G_Inc: "Receivable Goods Income",
  RL_P_Inc: "Receivable Product Income",

  VAT_Inc: "Goods and Product VATs",
  
  PD_Exp: "Paid purchase Expenses",
  RL_Exp: "Receivable purchase Expenses",
  WIZ_Exp: "WithHold Expenses",
...
function filterMethod(rows, terms) {
  ...
  const filteredRows = rows.filter((row, i) => {
    ...
    let sS1 = true; // quickFilter.quickOOptions.Stored && row.storeStatus == "Onstore"; //holding toggle_quicx search
    let sS2 = true; //quickFilter.quickOOptions.Taken && row.storeStatus == "taken";
