<template id="full-page">
  ...
  <div class="col row q-pa-xs q-gutter-xs" style="height: 90vh" v-if="_this_acctype ?? false">
    <div
    class="col-xs-12 col-sm-12 col-md-9 col-lg-grow col-xl-auto column rounded"
      style="height: 89vh"
    >
    ...
    <!--- Profiling...-->
    <div
    class="col-xs-11 col-sm-11 col-md col-lg-auto col-xl-auto column rounded shadow-2" v-show="selected[selected.length - 1]"
    >
      <q-card
...
import { supplierStore } from "stores/dataStores/supplierStore"; //supplier Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //supplier Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { supplierSchema } from "src/composables/schemas/procurmentSchemas";
...
import {useAssetFilter} from "src/composables/filters/supplierfilter"
var {
...
_thisModel.value = "supplier"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = supplierSchema

const supplierService = supplierStore();
const _thisService = thisStore();
const _thisapiUrl ='/procurment/supplier'
...
return supplierService.getstatus_Rows}) 
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(supplierService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(supplierService.getDatas)
});
...
onMounted(async () => {
  supplierService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  supplierService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
let quickFilterOptions = {
  ...
  quickOOptions: ["A", "AA", "AAA"],
};

//***************** ALTER THOSE ABOVE LABELS(OPTIONAL) &  THE sS1/2/3(must)*****/

let quickFilter = reactive({
  quickOOptions: { A: true, AA: true, AAA: true },
});
//-------------------------------supplierome Filter ---Function
//const showALL=computed(()=>{})//with out return
const filter = computed(() => {
  return {
    search: search.value,
    A: quickFilter.quickOOptions.A, //StoreStatusFilter
    AA: quickFilter.quickOOptions.AA,
    AAA: quickFilter.quickOOptions.AAA,
  };
}); //with out return
...
function filterMethod(rows, terms) {
  ...
  const filteredRows = rows.filter((row, i) => {
    ...
    let sS1 = quickFilter.quickOOptions.A && row.rateit == "A"; //holding toggle_quicx search
    let sS2 = quickFilter.quickOOptions.AA && row.rateit == "AA";
    let sS3 = quickFilter.quickOOptions.AAA && row.rateit == "AAA";
    let sS4 = false; //quickFilter.quickOOptions.Meter && row.UoM == "Meter";
...
let itemDetailColumns = reactive([
  ["date", "supplierTIN", "rateit"],
  ["supplierName", "supplierTIN", "supplierID","rateit", "reminderDays"],
  ["supplierBankAccount", "feedback", "address"],
]);
