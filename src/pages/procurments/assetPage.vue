<template id="full-page">
  ...
  <div class="col row  q-gutter-xs " style="height: 90vh" v-if="(_this_acctype ?? false) && (_profile.id ?? false)">
    ...
    >
        ...
        <q-card-section class="q-pa-none col column bordered wrap scroll bg-white"
        >
          <!----------------------------------------- Table View...-->
          <div class="col row">
            <q-table color="white"
...
<!-- {{ _this_Rows}}  -->
    ...
    <!----------------------------------VISUALITH-->
    <div  class="col row rounded"  v-else >
    ...
    <div class="col column">
     ...
     <BChart style="max-height:88vh;"
        ...
        :storeService="assetService.getyearDatas"
      />
    </div>
...
import { ref, reactive, computed, onMounted, onBeforeMount,onUnmounted, watch } from "vue";
...
_this_acctype =computed(()=>{
        if(Objprops._acctype ?? false){
          console.log('Model AccessType \n'+Objprops._acctype[_thisModel.value] ?? null)
...
if ($q.screen.lt.md) {
  ...
  //invisibleColumns.push("assetName");
}
if ($q.screen.lt.lg) {
...
function _this_RowsCompute(_Datas) {
  // pagination.value.rowsPerPage=_Datas.length5050> 50 ? _Datas/2: 25 // _pageSettings.value['tableView']=='cards' ? 0 : 3
  return _Datas
}
//syncSetting()
...
async function Create_this() {  //UpdateRowItem User DAta with no graphic to update
      ...
      console.log(_this.value,'creating asset')
...
let itemDetailColumns = reactive([
  ...
  ["model", "assetName", "tags", "ItemQs", "cost", "UoM", "userID"],
...
//-----------Search Box Based  Form ()
let search = ref("");
//-----------Slide Buttons Based Form
let filterDays = ref({min: 2,max: 8 })
//------------Buttons Based Form(2 step)
//step_1---- Form Data Labeling
let checkBoxLabel = { 
  group_I: ["Machines", "Consumable", "Acce", "Tools","Spares","Sanitary","Raws"],
  group_II: ["Issued","Stored","taken"],
};
//step_2----Form Data Settings
let checkBoxValue = reactive({
  group_I: { Machines: true, Consumable: true, Acce: true, Tools: true ,Spares: true,Sanitary:true,Raws:true},
  group_II: { Issued:true,Stored:true,taken:true },  
});
//-------------======= Searching Parameters and Value SEtting====END
...
const _thisfilter= computed(() => {
  return {
    search: search.value,
    //---
    Machines: checkBoxValue.group_I.Machines, //StoreStatusFilter
    Consumable: checkBoxValue.group_I.Consumable, //
    Acce: checkBoxValue.group_I.Acce,
    Tools: checkBoxValue.group_I.Tools,
    Spares: checkBoxValue.group_I.Spares,
    Sanitary: checkBoxValue.group_I.Sanitary,
    Raws: checkBoxValue.group_I.Raws,
   // Issued: checkBoxValue.group_I.Issued,
