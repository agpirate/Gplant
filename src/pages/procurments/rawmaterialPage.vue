<template id="full-page">
  ...
  <div class="col row  q-gutter-xs" style="height: 90vh" v-if="_this_acctype ?? false">
    ...
    style="height: 90vh" v-if="_pageSettings.vw_i"
    >
...
<!-- {{ _this_Rows}}  -->
                ...
                <q-tooltip class="bg-indigo" :offset="[10, 10]">
                  ...
                  >
                    ...
                    <template v-if="object.schema ?? false">
                        ...
                        >
                          <!--- else it is objects...-->
                          <div v-if="props.row[object.name] ?? false"> 
                          <q-list
    ...
    <div class="col column">
     ...
     <BChart style="max-height:88vh;"
        ...
        :storeService="rawmaterialService.getyearDatas"
      />
    </div>
  ...
  <q-page-sticky position="bottom" :offset="[3, 3]">
...
import { rawmaterialStore } from "src/stores/dataStores/rawmaterialStore"; //rawmaterial Store ( Main Store)
import { thisStore } from "stores/dataStores/thisStore"; //rawmaterial Store ( Main Store)
// import { genapiStore } from "src/stores/jstStores/genapiStore";

import { rawmaterialSchema } from "src/composables/schemas/procurmentSchemas";
...
import {useAssetFilter} from "src/composables/filters/rawmaterialfilter"
var {
...
const metaData = {
  // sets document title
  title: "YGP  rawmaterial Managment",
...
_thisModel.value = "rawmaterial"; // the Vue_Page_DataModel (Listing Collection Name)
//  var _thisModelHeader = ref(_thisModel.value+" Managment"); // the Vue_Page_Data_Headers(Descriptions...)
 _this_Schema.value = rawmaterialSchema

 const rawmaterialService = rawmaterialStore();
const _thisService = thisStore();
const _thisapiUrl ='/procurment/rawmaterial'
...
return rawmaterialService.getstatus_Rows}) 
...
if ($q.screen.lt.md) {
 ...
 // invisibleColumns.push("rawmaterialName");
 invisibleColumns.value.push("rawType");
}
if ($q.screen.lt.lg) {
  //invisibleColumns.push("storeStatus");
 // invisibleColumns.push("rawName");
  invisibleColumns.value.push("gmStatus");
  // invisibleColumns.push("assetOf");
  //invisibleColumns.push("catagories");
...
//==============Q1
 async function this_Query (_queKey=null,_value=null){
  ...
  _this_Query.value={}
  // _this_Rows_Sync()
  await Sync_this(rawmaterialService,_this_Query.value)
...
_this_Rows = computed(() => { //Updating is comming
  return _this_RowsCompute(rawmaterialService.getDatas)
});
...
onMounted(async () => {
  rawmaterialService.asyncAnualData().then((response) => {
    if (response) {
  ...
  // clearInterval(sync_this);
  rawmaterialService.set_syncLock(true)
  //-------
  //---
  this_Query()
});
...
let checkBoxLabel = {
  ...
  group_II: ["Issued","Stored","taken"],

};
//***************** ALTER THOSE ABOVE LABELS(OPTIONAL) &  THE sS1/2/3(must)*****/

let checkBoxValue = reactive({
  group_I: { c2h2:true,o2: true, co2: true,useable: false, unsold: false,facility:true },
  group_II: { Issued:true,Stored:true,taken:true },  
});
...
const _thisfilter = computed(() => {
  return {
   ...
   taken: checkBoxValue.group_II.taken,
...
let itemDetailColumns = reactive([
  ...
  ["financeStatus", "takenBy", "cost", "supplier", "storeStatus"],
]);
